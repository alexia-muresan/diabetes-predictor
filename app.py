...
for i, feature in enumerate(features):
    ...
    # Model expects lbs; user enters kg
        default_weight_kg = feature_means['weight'] * 0.45359237  # convert default (lbs→kg)
        val_kg = col.number_input(
            label,
            value=float(default_weight_kg),
            min_value=0.0
    )
        if val_kg == -1:
            patient[feature] = feature_means['weight'] / feature_stds['weight']
        else:
            val_lbs = val_kg / 0.45359237  # convert kg→lbs for normalization
            patient[feature] = (val_lbs - feature_means['weight']) / feature_stds['weight']

    elif feature == 'height':
    # Model expects inches; user enters cm
        default_height_cm = feature_means['height'] * 2.54  # convert default (in→cm)
        val_cm = col.number_input(
            label,
            value=float(default_height_cm),
            min_value=0.0
    )
        if val_cm == -1:
            patient[feature] = feature_means['height'] / feature_stds['height']
        else:
            val_in = val_cm / 2.54  # convert cm→in for normalization
            patient[feature] = (val_in - feature_means['height']) / feature_stds['height']
